...
async function updateOrdersView() {
    ...
    // try {
        ...
        data.forEach(order => {
            ...
            div_header.innerHTML = `Order: `+ order.id;
            const div_user = document.createElement('div');
            div_user.classList.add('order-header-user');
            div_user.innerHTML = `user: ${order.user_id}`;
            ...
            div_button.addEventListener('click', async () => {
                const response = await fetch('/order', {
                    ...
                    body: JSON.stringify({orderId: order.id, statusId: 4}),
                });
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
                }
